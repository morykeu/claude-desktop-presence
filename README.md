...
| ------- | ------- | -------- | -------- | ----- | -------- |
| idle    | 14      | 0.70     | **1.41** | 2.48  | **2.95** |
| working | 27      | **5.58** | **7.73** | 13.83 | 15.25    |

<!-- /generated:calibration-table -->

<!-- generated:calibration-provenance -->

_Measured 2026-09-07 on the target machine (12 cores), from the raw samples in `measurements/calibration-2026-09-07T15-31-15Z.json`. Generated by `npm run docs:sync` — do not edit by hand._
...
- **floor 0.82 %** — p5 of phase 1, where the rolling baseline settles at runtime
- **idle edge 2.67 %** (p95 of phase 1) · **work edge 6.19 %** (p5 of phase 2) → 3.52 points apart, the distributions do not overlap
- **BUSY above 4.43 %** — exactly midway between those two edges
- **back to idle at 3.10 %** — above the idle maximum of 2.95 %, so an ordinary fluctuation cannot keep the daemon latched in BUSY
- into the config: multiplier 2.5 · delta 3.6 · exitFactor 0.7
...
hypothetical here: an earlier measurement was recorded as min/median/p90/max, the
readings were discarded, and when the threshold rule later changed to key off two
different percentiles there was nothing left to compute them from. The table above had
to be published from a reconstruction until a run with its samples intact replaced it.
...
The measurement is the newest `calibration-*.json` in [`measurements/`](measurements/),
falling back to the reconstruction in [`src/measurement.ts`](src/measurement.ts) when
there is none. The line under the table says which one these numbers came from, and a
reconstruction adds a note about which of its figures are indicative — so the documents
state their own provenance rather than leaving it to prose here to stay true.
